...
public:
  ...
  template<class BaseT> std::vector<std::shared_ptr<BaseT>> ResolvePlugin()
  {
    ...
    for( std::multimap<std::type_index, std::function<std::shared_ptr<void>()>>::iterator it = ret.first; it != ret.second; ++it ) { return_plugins.push_back( std::static_pointer_cast<BaseT>( ( it->second )() ) ); }
